...
scope: {
    innerVar0: '@', // '@attr'  单项绑定
    innerVar1: '=', // '=attr'  双向绑定   使用组件时，如果不传值，在组件中对该值进行操作时报错，而且不能给配置对应的属性赋值时使用 {{}}
    innerVar2: '&'  // '&attr'  绑定函数
...
其中的 `$transclude` 参数数是实际被执行用来克隆元素和操作DOM的函数。在控制器内部操作DOM是和 `AngularJS` 风格相悖的做法，但通过链接函数就可以实现这个需求。仅在 `compile` 参数中使用 `transcludeFn` 是推荐的做法。 
